...
static void run(const char* source_path, const char* label, uid_t uid,
    ...
    LOG(WARNING) << "run func watch...";
    watch_package_list(&global);
    LOG(WARNING) << "run func watch end...";
...
static bool sdcardfs_setup(const std::string& source_path, const std::string& dest_path,
    ...
    std::string opts = android::base::StringPrintf(
        ...
        multi_user ? "multiuser," : "", derive_gid ? "derive_gid," : "",
        default_normal ? "default_normal," : "", unshared_obb ? "unshared_obb," : "", mask, userid, gid);
...
static void run_sdcardfs(const std::string& source_path, const std::string& label, uid_t uid,
                         gid_t gid, userid_t userid, bool multi_user, bool full_write,
                         bool derive_gid, bool default_normal, bool unshared_obb) {
    ...
    if (multi_user) {
        ...
        if (!sdcardfs_setup(source_path, dest_path_default, uid, gid, multi_user, userid,
            ...
            !sdcardfs_setup_bind_remount(dest_path_default, dest_path_write, AID_EVERYBODY, full_write ? 0007 : 0027)||
        ...
        if (!sdcardfs_setup(source_path, dest_path_default, uid, gid, multi_user, userid,
            ...
            !sdcardfs_setup_bind_remount(dest_path_default, dest_path_full, AID_EVERYBODY,
                                         0007)) {
            LOG(FATAL) << "failed to sdcardfs_setup";
        }
    }
    ...
    LOG(WARNING) << "run_sdcardfs func exit...";
    exit(0);
}
...
int main(int argc, char **argv) {
    ...
    if (should_use_sdcardfs()) {
        run_sdcardfs(source_path, label, uid, gid, userid, multi_user, full_write, derive_gid, default_normal, unshared_obb);
    } else {
