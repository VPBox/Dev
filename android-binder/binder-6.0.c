...
static struct binder_node *vm_binder_context_mgr_node;
static struct binder_node *get_other_binder_ns_context_mgr_node(void)
{
	if(current_drv_ns() != &init_drv_ns){
		return find_binder_ns(&init_drv_ns)->context_mgr_node;
	}else{
		return vm_binder_context_mgr_node;
	}
};
	...
	if (strong) {
		if (internal) {
			if (target_list == NULL &&
			    ...
			    !((node == node->proc->binder_ns->context_mgr_node || node == get_other_binder_ns_context_mgr_node()) &&
			    node->has_strong_ref)) {
						  ...
						  struct binder_node *node)
{
	...
	if(node != proc->binder_ns->context_mgr_node && node == get_other_binder_ns_context_mgr_node()){
		new_ref->desc = INIT_CONTEXT_MGR_HANDLE;
	}else{
			...
			binder_size_t *consumed)
{
	...
	while (ptr < end && thread->return_error == BR_OK) {
		...
		case BC_DECREFS: {
			...
			}else if (target == INIT_CONTEXT_MGR_HANDLE && get_other_binder_ns_context_mgr_node() &&
			    (cmd == BC_INCREFS || cmd == BC_ACQUIRE)) {
				ref = binder_get_ref_for_node(proc,
				       get_other_binder_ns_context_mgr_node());
				if (ref->desc != target) {
...
static long binder_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case BINDER_SET_CONTEXT_MGR:
		...
		if(current_drv_ns() != &init_drv_ns) vm_binder_context_mgr_node = proc->binder_ns->context_mgr_node;
		break;
