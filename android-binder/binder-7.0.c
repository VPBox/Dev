...
struct binder_device {
	...
	struct binder_context vmcontext;
};
...
struct binder_proc {
	...
	struct binder_context *context;
	struct binder_context *othersystemcontext;
	...
	if (strong) {
		if (internal) {
			if (target_list == NULL &&
				...
				!(node->proc && node->proc->othersystemcontext && node == node->proc->othersystemcontext->binder_context_mgr_node && node->has_strong_ref)) {
					...
					struct binder_ref *new_ref)
{
	...
	if(proc->othersystemcontext && node == proc->othersystemcontext->binder_context_mgr_node){
		new_ref->data.desc = INIT_OTHER_CONTEXT_MGR_HANDLE;
	}else{
			       ...
			       binder_size_t extra_buffers_size)
{
	...
	if (reply) {
		...
		if(proc->othersystemcontext && tr->target.handle == INIT_OTHER_CONTEXT_MGR_HANDLE){
			mutex_lock(&proc->othersystemcontext->context_mgr_node_lock);
			target_node = proc->othersystemcontext->binder_context_mgr_node;
			if (target_node == NULL) {
				return_error = BR_DEAD_REPLY;
				mutex_unlock(&proc->othersystemcontext->context_mgr_node_lock);
				return_error_line = __LINE__;
			...
			binder_size_t *consumed)
{
	...
	while (ptr < end && thread->return_error.cmd == BR_OK) {
		...
		case BC_DECREFS: {
			...
			ptr += sizeof(uint32_t);
			ret = -1;
			if (proc->othersystemcontext && increment && target == INIT_OTHER_CONTEXT_MGR_HANDLE) {
				struct binder_node *ctx_mgr_node;
				mutex_lock(&proc->othersystemcontext->context_mgr_node_lock);
				ctx_mgr_node = proc->othersystemcontext->binder_context_mgr_node;
				if (ctx_mgr_node)
...
static int binder_open(struct inode *nodp, struct file *filp)
{
	...
	if(current_drv_ns() == &init_drv_ns){
		...
		proc->othersystemcontext = &binder_dev->vmcontext;
	}else{
		proc->context = &binder_dev->vmcontext;
		proc->othersystemcontext = &binder_dev->context;
	}
...
static int __init init_binder_device(const char *name)
{
	...
	binder_device->vmcontext.binder_context_mgr_uid = INVALID_UID;
	binder_device->vmcontext.name = name;
	mutex_init(&binder_device->vmcontext.context_mgr_node_lock);
